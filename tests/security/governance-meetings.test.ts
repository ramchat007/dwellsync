...
describe("Phase 11.3 — Governance Committee Meetings & Proceedings", () => {
  ...
  describe("Meeting RBAC & Permission Matrix", () => {
    ...
    it("TREASURER holds MEETINGS_VIEW but NOT MEETINGS_MANAGE; COMMITTEE_MEMBER holds MEETINGS_MANAGE", () => {
      ...
      expect(roleHasPermission("COMMITTEE_MEMBER", PERMISSIONS.MEETINGS_MANAGE)).toBe(true);
    });
    ...
    it("Operational and external roles cannot manage meetings", () => {
  ...
  // ============================================================
  // 2. MEETING LIFECYCLE STATE MACHINE & TERMINAL IMMUTABILITY
  // ============================================================
  ...
  describe("Quorum Calculation Engine", () => {
    const calculateQuorum = (quorumRequired: number, attendees: Array<{ status: string }>) => {
      const presentCount = attendees.filter((a) => a.status === "PRESENT").length;
      return presentCount >= quorumRequired;
    };

    it("evaluates quorum_met as TRUE when present attendees meet required threshold", () => {
      const attendees = [
        { status: "PRESENT" },
        { status: "PRESENT" },
        { status: "PRESENT" },
    ...
    it("does not count EXCUSED or ABSENT attendees towards quorum", () => {
      const attendees = [
        { status: "ABSENT" },
        { status: "EXCUSED" },
        { status: "EXCUSED" },
    ...
    it("evaluates quorum_met as TRUE if quorum_required is 0", () => {
