# DwellSyncHub — Multi-Society Membership Model (Phase 1)
...
Example:
- User `Rahul Sharma` (`rahul@DwellSyncHub.internal`):
...
- The server checks the `DwellSyncHub_active_society` cookie, but verifies that the user holds an `ACTIVE` membership in that society before granting access.
