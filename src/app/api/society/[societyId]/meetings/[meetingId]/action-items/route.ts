...
import { CreateActionItemSchema } from "@/lib/validations/governance";
import { createMeetingActionItem } from "@/lib/governance/meetingService";
...
export async function POST(
  ...
  try {
    ...
    if (result.error || !result.actionItem) {
      return NextResponse.json(
        { error: result.error || "Failed to create action item" },
        { status: 400 }
      );
    }

    return NextResponse.json({ success: true, actionItem: result.actionItem }, { status: 201 });
  } catch (err: any) {
    console.error("[API/meetings/action-items/POST] Exception:", err);
    return NextResponse.json(
