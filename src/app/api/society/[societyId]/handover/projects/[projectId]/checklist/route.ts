...
import { CreateChecklistItemSchema } from "@/lib/validations/handover";
import { z } from "zod";
...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error("[API/handover/checklist GET]", error);
...
export async function POST(
  ...
  try {
    ...
    if (error) {
      console.error("[API/handover/checklist POST]", error);
    ...
    await recordAuditLog({
      ...
      societyId,
      action: "HANDOVER_CHECKLIST_CREATED",
      resourceType: "handover_checklist_item",
      resourceId: item.id,
      metadata: { title: item.title, category: item.category, project_id: projectId },
    });
